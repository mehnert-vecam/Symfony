...
class AddrPersonController extends Controller
{
    public function indexAction($id='')
    {
      ...
      if($id== ''){   // wurde keine ID uebergeben, handelt es sich um einen neuen Eintrag, für den ein neues Objekt erzeugt werden muss.     
        ...
        $form = $this->createFormBuilder($person) // Formular wird auf Basis der Person erstellt   
          ...
          return $this->render('VestSymfonyBundle:Default:AddrPerson.html.twig', array(
    ...
    public function saveAction(Request $request)
    {
        ...
        if($id >0){   // checkt, ob die ID gesetzt ist. Ist sie gesetzt, wird bearbeitet
          $repository = $this->getDoctrine()->getRepository('VestSymfonyBundle:AddrPersonen');      // lädt das repository und sagt ihm um welche Entity es sich kümmern soll.
         ...
         */                  
        return $this->render('VestSymfonyBundle:Default:erfolg.html.twig', array('ergebnis' => $ergebnis, 'id' => $id, 'error' => $error));  
